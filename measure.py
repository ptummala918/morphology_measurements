...
astr_dists_all = np.zeros((frame_count, len(astr_vertices)), dtype=np.float32)
calc_dists_all = np.zeros((frame_count, len(calc_vertices)), dtype=np.float32)
...
for frame in range(frame_count):
    ...
    # store heatmap minimum distances
    astr_dists_all[frame] = pair_distances.min(axis=1)
    calc_dists_all[frame] = pair_distances.min(axis=0)
    ...
    distances.append({'frame': frame + 1, 'min_distance': min_distance, 'center_distance': center_distance, 'contact_percentage' : astr_contact_pct, 
                      'coc_x': coc[0], 'coc_y': coc[1], 'coc_z': coc[2]})
...
plt.title(f'Subtalar Joint Contact Area Over Time with Threshold of {threshold} mm', fontsize=14)
plt.xlabel('Frame', fontsize=12)
...
plt.figure(figsize=(10, 6))
plt.plot(distances_dataframe['frame'], distances_dataframe['coc_x'], label='Coc X-axis', color='blue', linewidth=2)
plt.plot(distances_dataframe['frame'], distances_dataframe['coc_y'], label='Coc Y-axis', color='red', linewidth=2)
plt.plot(distances_dataframe['frame'], distances_dataframe['coc_z'], label='Coc Z-axis', color='yellow', linewidth=2)
...
# save heatmap data
np.save('data/astr_vertex_distances.npy', astr_dists_all)
np.save('data/calc_vertex_distances.npy', calc_dists_all)